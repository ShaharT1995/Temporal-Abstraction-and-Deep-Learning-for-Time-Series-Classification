...
def fill_transformation1(arr, path, file_type, classes, time_series_number):
        ...
        with open(ta_output) as file:
            ...
            for index in range(2, len(lines), 2):
                ...
                for info in range(len(data) - 1):
                    ...
                    arr[int(entity_id)][int(parse_data[0]) - 1: int(parse_data[1]) - 1, time_series_number] = parse_data[2]
...
def transformation_1(path, gradient_path, output_path, file_type, number_of_rows, number_of_columns, classes):
    ...
    arr = fill_transformation1(arr, path, file_type, classes, 0)
    arr = fill_transformation1(arr, gradient_path, file_type, classes, 1)
