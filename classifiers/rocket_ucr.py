...
class Base_Classifier_ROCKET(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, n_kernels=10_000, kss=(7, 9, 11), verbose=True):
        ...
        self.kernels_ = None
        self.x_train_ = None
        self.train_timings_ = None
        self.ridge_cv_ = None
        self.x_test_ = None
        self.test_timings_ = None
    ...
    def fit(self, x, y):
        ...
        time_a = time.perf_counter()
        self.ridge_cv_ = RidgeClassifierCV(alphas=np.logspace(-3, 3, 10), normalize=True)
...
class Classifier_Rocket:
    ...
    def fit(self, x_train, y_train, x_test, y_val, y_true, iteration):
        ...
        acc = rocket.score(x_test, y_true)
        ...
        df_timings = pd.DataFrame([train_timings, test_timings], index=['train', 'test'],
                                  columns=['x_transform', 'ridge_operation', 'total'])
        # if self.verbose:
        #     print(df_timings)
