...
from constants import TemporalAbstractionParamsColumns, MethodsNames, TimeIntervalsColumns, StatesColumns
from kfold.kfold import KFold
import time


def regular_system_flow(files_manager, preprocessing_params_df, temporal_abstraction_params_df, states=None, ncr=False):
    """
...
def kfold_system_flow(nb_folds, files_manager, preprocessing_params_df, temporal_abstraction_params_df, states=None,
    ...
    for (train_fm, test_fm) in fms:
        ...
        regular_system_flow(train_fm, preprocessing_params_df, temporal_abstraction_params_df, states, ncr)
...
def system_flow(files_manager, preprocessing_params_df, temporal_abstraction_params_df, states=None, kfold=None,
    ...
    if kfold:
        kfold_system_flow(kfold, files_manager, preprocessing_params_df, temporal_abstraction_params_df, states, ncr)
    else:
        regular_system_flow(files_manager, preprocessing_params_df, temporal_abstraction_params_df, states, ncr)
    end_time = time.time()
