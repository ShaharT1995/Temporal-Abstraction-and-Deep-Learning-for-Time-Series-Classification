...
def concat_results(path_ta_dir, raw_data = False):
    ...
    "cohen_kappa",'duration',"f1_score_macro", "f1_score_micro","f1_score_weighted"
    ,"iteration"]
    ...
    if os.path.exists('Unified_results/results_of_all_combinations.csv'):
        df.to_csv("Unified_results/results_of_all_combinations.csv", index=False, mode='a', header=0)
    else:
        df.to_csv('Unified_results/results_of_all_combinations.csv', index=False)
...
def create_all_graphs():
    config = ConfigClass()
    path_for_ta_dir = config.path_after_TA + config.archive +"//"
    # run only once
    concat_results(path_for_ta_dir)
    # path_for_raw_dir = config.path + "ResultsProject//RawData//"+ config.archive +"//"
    # concat_results(path_for_raw_dir)
    #
