...
def graph_1(df_after_ta, path_file_raw_data):
    ...
    df_after_ta = df_after_ta.loc[(df_after_ta.paa == 1)&((df_after_ta.dataset_name == "Computers"))]
    ...
    g = sns.FacetGrid(data,row="Dataset Name", col="Evaluation Metric", hue="Data", height=6, aspect=1.2,sharey=False)
    g = g.map(sns.pointplot, "classifier_name", "value", ci=95, err_style="bars", join=False, s=20)
...
def graph_2(df_after_ta, path_file_raw_data):
    ...
    melt_df_after = pd.melt(df_after_ta, id_vars=["method", "paa"],
                            value_vars=['F_Score', "Accuracy", "Learning Time (ms)"])
    ...
    g = sns.FacetGrid(melt_df_after, col="Evaluation Metric", hue="paa", height=6, aspect=1.3, sharey=False)
    g = (g.map(sns.pointplot, "method", "value", ci=95, join=False))
    plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0., title="PAA")
...
def create_all_graphs():
    path = "C:\\Users\\hadas cohen\\Desktop\\ResultsAfterTA\\"
    #concat_results(path)
    graph_2("results_new.csv", "C:\\Users\\hadas cohen\\Desktop\\raw_data_results.csv")
